...
def printsuccess(route: str, success_type: str = ""):
    ...
    if show_success:
        print(f"\1xb[32;1mCorrect {success_type}"+spaces_count, f"at {route}\x1b[0m")
...
class Test:
    ...
    def __call__(self):
        if   self.method == "GET": # Could use match case
            resp = requests.get(self.route, self.request_data)
        elif self.method == "POST":
            resp = requests.post(self.route, self.request_data)
        else:
        ...
        if self.status_code is None:
            if should_error:
                if resp.staus_code == 200:
                    printfail("Expected error but response code is 200", self.route, "STATUSCODE")
        else:
            if self.status_code != resp.status_code:
                printfail(f"Wrong status code {resp.status_code}, it should be {self.status_code}", self.route, "STATUSCODE")
            else:
        ...
        if self.result == "error" or self.result == "err":
            response_error
        return valid_status_code and valid_response


def g(route: str, result="error", status_code: int = None): # GET request
    tests.append(Test(route=route, result=result, request_data=request_data, status_code=status_code))
